(function (namespace, $) {
    ...
    p.initialize = function () {
        ...
        this._loadUsers(1, "#txtelabororef");
        this._loadUsers(2, "#txtejecutivoref");
        this._loadTransfers();
    }

    p._loadIntervalProgramacion = function () {
        var refreshIntervalMessages = setInterval(function () {
    ...
    p._createdomgrid = function (ctrlgrid, btnprogtraf, btnexporta, ctrlmenu) {
        ...
        $(ctrlgrid).jqxGrid(
        {
            ...
            rendertoolbar: function (toolbar) {
                ...
                html += '	<input type="button" value="Programar trafico" id="' + btnprogtraf + '" />';
                html += '	<input type="button" value="Exportar a Excel" id="' + btnexporta + '" />';
                html += '</div>';
                var $new = $(html);
                toolbar.append($new);
                $("#" + btnprogtraf).jqxButton({ theme: theme });
                $("#" + btnprogtraf).on('click', function () {
                    bootbox.confirm({
                        message: "El trafico a programar pertenece a una remesa?",
                        buttons: {
                            confirm: {
                                label: 'Si',
                            },
                ...
                var contextMenu = $(ctrlmenu).jqxMenu({ width: 240, height: 390, autoOpenPopup: false, mode: 'popup', theme: theme });
                ...
                $(ctrlmenu).on('itemclick', function (event) {
                    ...
                    if ($.trim($(args).text().toLowerCase()) == "capturar itn") {
                    ...
                    else if ($.trim($(args).text().toLowerCase()) == "capturar doda") {
                        $('#txtdodaspan').text(referencia);
                        $('#formdoda').modal('show');
                    }
                    ...
                    else if ($.trim($(args).text().toLowerCase()) == "capturar estatus pedimento") {
                        $('#txtstatusspan').text(dataRecord.tpreferencia);
                        $('#formstatus').modal('show');
                    }
                    else if ($.trim($(args).text().toLowerCase()) == "capturar estatus bodega") {
                        $('#txtestatusbodspan').text(dataRecord.tpreferencia);
                        $('#formestatusbod').modal('show');
                    }
                    else if ($.trim($(args).text().toLowerCase()) == "capturar estatus aduana") {
                    ...
                    else if ($.trim($(args).text().toLowerCase()) == "asignar transfer") {
                        $('#txttransferspan').text(dataRecord.tpreferencia);
                        $('#formtransfer').modal('show');
                    }
                    else if ($.trim($(args).text().toLowerCase()) == "asignar hora estimada") {
                        $('#txthoraspan').text(dataRecord.tpreferencia);
                        $('#formhora').modal('show');
                    }
                        ...
                        if (conf) {
                            if (dataRecord.tpreferencia != "") {
                                p._desprogramatr(dataRecord.tpreferencia);
                            }
                            else {
                                p._desprogramatr(dataRecord.tpentrada);
                            }
                        }
                    }
                });
    ...
    p._loadCrucesDiarios = function () {
        $.ajax({
            ...
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            url: 'services/dashboard.asmx/loadcrucesdiarios',
            success: function (data) {
                var obj = $.parseJSON(data.d).Table[0];

                var datacruces = [{ y: moment().subtract(6, 'days').format('YYYY-MM-D'), a: obj.CRU6, b: obj.CRU6EXP },
                      { y: moment().subtract(5, 'days').format('YYYY-MM-D'), a: obj.CRU5, b: obj.CRU5EXP },
                      { y: moment().subtract(4, 'days').format('YYYY-MM-D'), a: obj.CRU4, b: obj.CRU4EXP },
                      { y: moment().subtract(3, 'days').format('YYYY-MM-D'), a: obj.CRU3, b: obj.CRU3EXP },
                      { y: moment().subtract(2, 'days').format('YYYY-MM-D'), a: obj.CRU2, b: obj.CRU2EXP },
                      { y: moment().subtract(1, 'days').format('YYYY-MM-D'), a: obj.CRU1, b: obj.CRU1EXP },
                      { y: moment().format('YYYY-MM-D'), a: obj.CRU0, b: obj.CRU0EXP }];
    ...
    p._enableEvents = function () {
        ...
        $("#cmdcapturadoda").click(function () {
            var referencia = $("#txtdodaspan").text().toUpperCase();
            if (referencia == "") {
                bootbox.alert("No ha seleccionado Referencia");
                return;
            }
            var coments = $("#txtdodaref").val().toUpperCase();
            if (coments == "") {
                bootbox.alert("No ha asignado DODA");
                return;
            }
            ...
            $.ajax({
                data: JSON.stringify(data),
                dataType: 'json',
                url: 'services/dashboard.asmx/actualizadoda',
                type: 'POST',
                contentType: 'application/json; charset=utf-8',
                success: function (result) {
                    p._refreshgridprogramacion();
        ...
        $("#cmdcapturahora").click(function () {
            var referencia = $("#txthoraspan").text().toUpperCase();
            if (referencia == "") {
                bootbox.alert("No ha seleccionado Referencia");
                return;
            }
            var coments = $("#txthoraref").val().toUpperCase();
            if (coments == "") {
                bootbox.alert("No ha asignado Hora");
                return;
            }
            ...
            $.ajax({
                data: JSON.stringify(data),
                dataType: 'json',
                url: 'services/dashboard.asmx/actualizhoraestimada',
                type: 'POST',
                contentType: 'application/json; charset=utf-8',
                success: function (result) {
                    p._refreshgridprogramacion();
        ...
        $("#cmdcapturatransfer").click(function () {
            var referencia = $("#txttransferspan").text().toUpperCase();
            if (referencia == "") {
                bootbox.alert("No ha seleccionado Referencia");
                return;
            }
            var coments = $("#txttransferref").val();
            if (coments == "") {
                bootbox.alert("No ha asignado transfer");
                return;
            }
            ...
            $.ajax({
                data: JSON.stringify(data),
                dataType: 'json',
                url: 'services/dashboard.asmx/actualiztransfer',
                type: 'POST',
                contentType: 'application/json; charset=utf-8',
                success: function (result) {
                    p._refreshgridprogramacion();
                    ...
                    $("#txttransferspan").text('');
                    $("#txttransferref").val('')
                    $('#formtransfer').modal('hide');
                }
            });
            return false;
        });
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (datares) {
                        ...
                        bootbox.alert("Atencion!! El trafico no se ha encontrado en el sistema, al programarlo se asignara por default como Adelantado")
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (datares) {
                        ...
                        bootbox.alert("Atencion!! La entrada no se ha encontrado en el sistema, al programarlo se asignara por default como Adelantado")
                        $("input[name=inlineRadioOptions][value=" + 5 + "]").prop('checked', true);
                    }
        ...
        $("#cmdcapturaconsolidado").click(function () {
            ...
            var consolidado = $("#txtconsco").text();
    ...
    p._loadUsers = function (tipousuario, cbousuario) {
        var datos = {
        ...
        $.ajax({
            ...
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            url: 'services/dashboard.asmx/GetUsuarios',
            success: function (data) {
                $(cbousuario).empty().append('<option value="0"></option>');

                var json = $.parseJSON(data.d);
                $.each(json, function (key, value) {
                    $(cbousuario).append($('<option>').text(value.userfname + " " + value.userlname).attr('value', value.userid));
                });
            }, error: function (xhr, textStatus, error) {
                var errorM = $.parseJSON(xhr.responseText);
    ...
    p._loadTransfers = function () {
        ...
        $.ajax({
            ...
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            url: 'services/dashboard.asmx/GetTransfers',
            success: function (data) {
                $("#txttransferref").empty().append('<option value="0"></option>');

                var json = $.parseJSON(data.d);
                $.each(json, function (key, value) {
                    $("#txttransferref").append($('<option>').text(value.ctnombre + " - CAAT:" + value.ctcaat).attr('value', value.ctid));
                });
            }, error: function (xhr, textStatus, error) {
                var errorM = $.parseJSON(xhr.responseText);
                bootbox.alert(errorM.Message);
